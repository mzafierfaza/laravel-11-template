...
<div class="card">
  <div class="card-header">
    <h4><i class="fa fa-trophy"></i> {!! __('Aksi Ekspor <small>(Server Side)</small>') !!}</h4>
    <div class="card-header-action">
      ...
      <table class="table table-striped table-hovered" id="datatable" @if ($canExport) data-export="true" data-title="{{ $title }}" @endif>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $item->title }}</td>
            <td>{{ $item->level }}</td>
            <td>{{ $item->countEnrollments() . ' Peserta' }}</td>
            ...
            <td>
              @include('stisla.includes.forms.buttons.btn-view', ['link' => route('competences.show', [$item->id])])
              @if ($canUpdate)
              @include('stisla.includes.forms.buttons.btn-edit', ['link' => route('competences.edit', [$item->id])])
              @endif
              @if ($canDelete)
              @include('stisla.includes.forms.buttons.btn-delete', ['link' => route('competences.destroy', [$item->id])])
              @endif
              @if (($canApprove || $canBypass) && $item->approved_status == 0)
              @include('stisla.includes.forms.buttons.btn-approve', ['link' => route('competence.approve', ['competence' => $item, 'approved_status' => 1])])
              @include('stisla.includes.forms.buttons.btn-reject', ['link' => route('competence.approve', ['competence' => $item, 'approved_status' => 2])])
              @endif
            </td>
          </tr>
          @endforeach
...
@else
@include('stisla.includes.others.empty-state', ['title' => 'Data ' . $title, 'icon' => 'trophy', 'link' => $routeCreate])
@endif
</div>
