      ...
      <div class="col-12">
        ...
        @if ($data->count() > 0)
          @if ($canExport)
            <div class="card">
              <div class="card-header">
                <h4><i class="fa fa-list"></i> {!! __('Aksi Ekspor <small>(Server Side)</small>') !!}</h4>
                <div class="card-header-action">
                  ...
                  <tbody>
                    @foreach ($data as $item)
                      <tr>
                        ...
                        <td>
                          @if ($canUpdate)
                            @include('stisla.includes.forms.buttons.btn-edit', ['link' => route('question-options.edit', [$item->id])])
                          @endif
                          @if ($canDelete)
                            @include('stisla.includes.forms.buttons.btn-delete', ['link' => route('question-options.destroy', [$item->id])])
                          @endif
                        </td>
                      </tr>
                    @endforeach
        ...
        @else
          @include('stisla.includes.others.empty-state', ['title' => 'Data ' . $title, 'icon' => 'list', 'link' => $routeCreate])
        @endif
      </div>
